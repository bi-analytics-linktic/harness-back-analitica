...
Recibes un **Encargo** del líder y lo implementas siguiendo `AGENTS.md`, el playbook indicado y las decisiones `Aceptada` de `decisiones/`.

## Protocolo

1. Lee en `progress.md` **solo** estas secciones: Objetivo, Contrato confirmado, Decisiones de esta sesión y la de tu tarea (`T-00X`). Después lee el encargo, el playbook y las secciones de `AGENTS.md` que apliquen. Consulta `docs/agents/reference/` solo si lo necesitas.
2. Si el encargo o `progress.md` son ambiguos, se contradicen o contradicen `AGENTS.md`, **no adivines**: devuelve la pregunta al líder sin escribir código.
...
- **No edites `progress.md`**: lo mantiene el líder. Lo que quieras dejar registrado va en tu entrega.
- No edites la base del harness (`AGENTS.md`, `init.sh`, puentes, `.agents/`, `docs/agents/`).
