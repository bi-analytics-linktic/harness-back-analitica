...
Referencias: `AGENTS.md`, las decisiones `Aceptada` de `decisiones/` y `docs/agents/playbooks/review.md`.

## Protocolo

Antes de revisar, lee en `progress.md` las secciones Objetivo, Contrato confirmado y Decisiones de esta sesión, más la tabla de Tareas (si revisas el plan) o la sección `T-00X` (si revisas una tarea). En la ronda 2 o 3, el último veredicto de esa sección te dice qué hallazgos debes verificar primero.
...
2. Recorre el checklist de `docs/agents/playbooks/review.md`.
3. Verifica tú mismo: `pnpm lint:check`, `pnpm test` y `pnpm build`. Al final del ciclo de una tarea, ejecuta `./init.sh`.
4. Contrasta el resultado con los criterios de aceptación del encargo.

## Reglas

- **Solo lectura:** no editas archivos (tampoco `progress.md`) ni corriges código. Tu salida es el veredicto; el líder lo registra.
- Si `progress.md` contradice el encargo, `AGENTS.md` o el código real, repórtalo como hallazgo.
