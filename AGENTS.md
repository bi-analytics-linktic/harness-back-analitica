...
Fuente única de instrucciones para cualquier agente IA (Claude Code, Codex, Gemini CLI, Cursor u otro). `CLAUDE.md`, `GEMINI.md` y `.cursor/rules/` solo apuntan aquí. Los detalles de consulta están en `docs/agents/reference/` y se leen **solo cuando la tarea lo requiere**.

> **Base del harness: no se modifica a medida que el proyecto crece.** Toda decisión posterior va en `decisiones/` (§17). Lee `decisiones/README.md` antes de trabajar.

## 0. Antes de empezar: `./init.sh`

Ejecuta `./init.sh` **antes de cualquier cambio**. Verifica el harness, la estructura y el toolchain, las reglas de SQL, lint, formato, build, tests y `/health` (cada base de datos y la caché).
...
- Al terminar una tarea, `./init.sh` debe pasar de nuevo.
...
**Dependencias aprobadas:** las de la tabla, `pino-http`, `@types/pg` y las que genera `nest new`. Cualquier otra requiere aprobación humana registrada en `decisiones/`. La base del proyecto se crea con la skill `nest-base`.

## 3. Comandos

```bash
./init.sh                          # verificación: SIEMPRE antes y después de trabajar
...
Raíz: `init.sh`, `progress.md` (estado de trabajo, no versionado), `postman/`, `FRONT.md`, `test/` (e2e), `.env.example`, `Dockerfile`, `azure-pipelines.yml`, `.nvmrc`, ESLint y Prettier. Harness: `.agents/skills/`, `.agents/agents/`, `docs/agents/` y `decisiones/`.
...
- **Conexiones:** una o varias, nombradas en env (`DATABASES=MAIN,REPORTS` + `DB_<NOMBRE>_*`). Se inyectan con `getPoolToken('<NOMBRE>')` y cada repository declara la suya. Variables: `docs/agents/reference/configuracion.md`.
...
- **Detalle** (clave, `X-Cache`): `docs/agents/reference/contrato-http.md`.
...
- **Detalle:** `docs/agents/reference/contrato-http.md`.
...
- **Ejemplos y tabla de códigos:** `docs/agents/reference/contrato-http.md`.
...
3. **Definition of Done:**
   ...
   - [ ] Decisión nueva registrada en `decisiones/` (si aplica)
   - [ ] `./init.sh` en verde
...
**Multiagente:** el `lider` orquesta y habla con el humano, el `implementador` escribe el código y el `revisor` audita el plan y cada entrega. Máximo 3 rondas de revisión por tarea; después se escala al humano. Definiciones y modelos recomendados en `.agents/agents/`. **Estado compartido:** `progress.md` en la raíz. Solo lo escribe el líder, desde `docs/agents/reference/progress-plantilla.md`; los subagentes leen solo las secciones que necesitan.

## 16. Playbooks y referencias

| Tarea | Archivo |
|---|---|
| Orquestar con agentes | `.agents/agents/lider.md`, `implementador.md`, `revisor.md` |
| Crear la base NestJS | `.agents/skills/nest-base/SKILL.md` (skill `nest-base`) |
| Crear un endpoint | `docs/agents/playbooks/new-endpoint.md` |
| Crear un módulo o cliente | `docs/agents/playbooks/new-module.md` |
| Agregar una conexión de BD | `docs/agents/playbooks/add-database.md` |
| Revisar un cambio | `docs/agents/playbooks/review.md` |
| Variables de entorno | `docs/agents/reference/configuracion.md` |
| Envelope, errores, auth, health | `docs/agents/reference/contrato-http.md` |
| Registrar una decisión | `decisiones/README.md` |

## 17. Decisiones (base inmutable)

La base del harness (`AGENTS.md`, `init.sh`, los archivos puente, `.agents/` y `docs/agents/`) **no se edita para reflejar decisiones del proyecto.** Cada decisión (dependencia, excepción, convención, regla por cliente, cambio de contrato) es un archivo en `decisiones/`:
- **Formato:** `decisiones/NNNN-titulo-kebab.md`, desde `_plantilla.md`, y agregada al índice de `decisiones/README.md`.
